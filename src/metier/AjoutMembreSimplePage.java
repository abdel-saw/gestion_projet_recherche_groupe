...
public class AjoutMembreSimplePage extends javax.swing.JFrame {
    ...
    private void ValiderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ValiderActionPerformed
        try {
            ...
            // Step 2: Prepare and execute SQL query to check existing members
            PreparedStatement ps = conn.prepareStatement(
                "SELECT username, nom, prenom FROM utilisateur WHERE idUtilisateur IN " +
                "(SELECT IdUtilisateur FROM utilisateur_groupe WHERE IdGroupe IN " +
                "(SELECT IdGroupe FROM groupe WHERE nom_groupe LIKE ?))");
            ps.setString(1, NomCourt);
            rs = ps.executeQuery();

            // Step 3: Clear table model and add column names
            ...
            // Step 4: Populate table model with query results
            while (rs.next()) {
            ...
            // Step 5: Check if user already exists in the group
            ...
            for (int c = 0; c < tableModel.getRowCount(); c++) {
                if ((Champ_username.getText().equals(tableModel.getValueAt(c, 0))) &&
                    (champ_nom.getText().equals(tableModel.getValueAt(c, 1))) &&
                    (champ_prenom.getText().equals(tableModel.getValueAt(c, 2)))) {
                    cp++;
                    break;
                }
            }
            if (cp == 1) {
                ...
                JOptionPane.showMessageDialog(null, "L'utilisateur est déjà un memebre simple du projet!!", "Incorrect Information", JOptionPane.ERROR_MESSAGE);
            } else {
                ...
                for (int c = 0; c < tableMode2.getRowCount(); c++) {
                    if ((Champ_username.getText().equals(tableMode2.getValueAt(c, 1))) &&
                        (champ_nom.getText().equals(tableMode2.getValueAt(c, 2))) &&
                        (champ_prenom.getText().equals(tableMode2.getValueAt(c, 3)))) {
                        cp1++;
                ...
                if (cp1 == 1) {
                    ...
                    PreparedStatement ps2 = conn.prepareStatement(
                        "INSERT INTO invitation_demande(host, user, IdGroupe, IdProject, nom_Projet, date, etat, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
